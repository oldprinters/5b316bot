...
selectAction.help(ctx => {
    ctx.replyWithHTML('<b><u>Основное меню слева от поля ввода</u></b>\n' +
        ...
        'Завтра [сообщение] - в 10:00\nЗавтра утром [сообщение] - в 08:00\nЗавтра днем [сообщение] - в 14:00\nЗавтра вечером [сообщение] - в 17:00\n'+
...
//------------------------------------------- завтра без времени
selectAction.hears(/^(завтра|Завтра) (утром )([ _.,а-яА-ЯйЙёЁa-zA-Z0-9+-=<>])*/, async ctx => {
    await tomorrowRemT(ctx, '8:00', 13)
})
//------------------------------------------- завтра без времени
selectAction.hears(/^(завтра|Завтра) (днем )([ _.,а-яА-ЯйЙёЁa-zA-Z0-9+-=<>])*/, async ctx => {
    await tomorrowRemT(ctx, '14:00', 12)
})
//------------------------------------------- завтра без времени
selectAction.hears(/^(завтра|Завтра) (вечером )([ _.,а-яА-ЯйЙёЁa-zA-Z0-9+-=<>])*/, async ctx => {
    await tomorrowRemT(ctx, '17:00', 15)
})
//-------------------------------------------
